...
public partial class MainViewModel : ObservableObject
{
    ...
    private Task<OperateResult<byte[]>?> SetAmplitude_1 ()
    {
        var result = Task. Run(() => DKS?.ACS. SetAmplitude(channel_1 , setAmplitudeValue_1));
        return result;
    }
    [RelayCommand(CanExecute = nameof(CanExecute_2))]
    private Task<OperateResult<byte[]>?> SetAmplitude_2 ()
    {
        var result = Task. Run(() => DKS?.ACS. SetAmplitude(channel_2 , setAmplitudeValue_2));
        return result;
    }
